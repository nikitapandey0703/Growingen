  ...
  return (
    <section className="group relative overflow-hidden bg-transparent">
      ...
      <SectionWrapper className="relative pt-8 sm:pt-10 md:pt-12 lg:pt-10 xl:pt-8 2xl:pt-12">
        {/* 
          Maintains text-top / image-bottom stacked layout up to lg (1024px).
          Switches to side-by-side at lg with our precise column fractions.
        */}
        <div className="grid items-center gap-10 sm:gap-12 md:gap-16 lg:min-h-[calc(100vh-140px)] lg:grid-cols-[minmax(0,1.3fr)_minmax(260px,0.7fr)] lg:gap-x-10 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.65fr)] xl:gap-x-12 2xl:grid-cols-[minmax(0,1.4fr)_minmax(340px,0.6fr)] 2xl:gap-x-16">
          ...
          {/* Left Text Container */}
          <div className="relative z-40 mx-auto flex w-full max-w-[34rem] flex-col text-center sm:max-w-[40rem] md:max-w-[48rem] lg:mx-0 lg:max-w-none lg:pr-8 xl:pr-12 lg:text-left">
            <p className="text-[8px] font-medium uppercase tracking-[0.18em] text-[var(--color-text-muted)] sm:text-[9px] md:text-[10px] lg:text-[11px] xl:text-[11px] 2xl:text-[12px]">
              Financial Platform Revamp & Application Development
            </p>

            <h1 className="mt-4 w-full max-w-[16ch] sm:max-w-[17ch] md:max-w-[18ch] lg:max-w-[22ch] xl:max-w-[23ch] 2xl:max-w-[24ch] font-[var(--font-heading)] text-[length:var(--fs-hero-title)] font-bold leading-[1.04] tracking-[-0.05em] text-[var(--color-text)]">
              {/* Removed w-full to prevent invisible overflow shifting the visual center */}
            ...
            {/* Explicitly centered on md, widened max-w to 65ch for elegant iPad line-breaks */}
            <p className="mx-auto mt-5 w-full max-w-[40ch] text-center text-[length:var(--fs-hero-subtitle)] font-medium leading-[1.7] text-[var(--color-text-muted)] sm:max-w-[48ch] md:max-w-[65ch] lg:mx-0 lg:max-w-[85ch] lg:text-left xl:max-w-[95ch]">
            ...
            {/* Meta tags gracefully spread into 4 centered columns on iPads (md) */}
            <div className="mt-10 grid w-full grid-cols-2 gap-x-4 gap-y-7 sm:mt-12 sm:gap-x-6 md:grid-cols-4 md:gap-x-8 lg:mt-14 lg:flex lg:flex-nowrap lg:items-start lg:justify-between lg:gap-5 xl:gap-8 2xl:gap-10">
              {heroMeta.map(([label, value]) => (
                <div key={label} className="min-w-0 text-left md:text-center lg:text-left lg:flex-auto">
          ...
          {/* Right Image Container - Centered entirely on md */}
